...
import {
  colModel,
  colName
} from '@/components/grid-headers/budget.rid.header.edit';
...
const Zone: NextPage = () => {
  ...
  if (
    ...
    allZone = 99;
    zonelink = `/api/budget/zone-project/${id}`;
  } else {
    // eslint-disable-next-line unused-imports/no-unused-vars
    allZone = getZone;
    zonelink = `/api/budget/zone-project/${getZone}`;
  }
    ...
    >
      <div className='grid grid-cols-1 gap-6 xl:grid-cols-1'>
        <Budgets
          ...
          isDel={false}
          isAdd={false}
          isCaption={`งบประมาณรายบุคคลเขตส่งเสริมที่ ${getZone}`}
        />
      </div>
    </Main>
  );
