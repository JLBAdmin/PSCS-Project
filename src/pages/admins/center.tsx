...
import {
  colModel,
  colName
} from '@/components/grid-headers/budget.center.header';
...
const Budget: NextPage = () => {
  ...
  useEffect(() => {
    ...
    if (['center', 'Center', 'all', 'All'].includes(getZone) === false) {
      // eslint-disable-next-line no-alert
      alert('คุณไม่ได้รับสิทธิ์ให้ทำงานหน้านี้');
      router.push('/budgets');
    }
  }, []);
  const { data, error } = useSWR([`/api/budget/noProve`, token], fetcher, {
    refreshInterval: 0
  });
    ...
    >
      <div className='grid grid-cols-1 gap-6 xl:grid-cols-1'>
        <Budgets
          ...
          isExport={true}
          isOption={action}
          isCaption={'งบประมาณรายบุคคล - ธุรการฝ่ายอ้อย'}
        />
      </div>
    </Main>
  );
