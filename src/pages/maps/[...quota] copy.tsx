...
const Index: NextPage = () => {
  ...
  const { data: allPoint, error: errPoint } = useSWR(
    `/api/geo/get-point/${quota}`,
    fetcher,
    {
      refreshInterval: 0
    }
  );
  const { data: Plot, error: errPlot } = useSWR(
    `/api/geo/get-plot/${quota}`,
    fetcher,
    {
      refreshInterval: 0
    }
  ...
  if (errPoint || errPlot) return <div>failed to load</div>;
  if (!allPoint || !Plot) return <div>loading...</div>;
    ...
    <>
      <Main meta={<Meta title='PS - พื้นที่อ้อย' description='Lorem ipsum' />}>
        <div className='grid grid-cols-1 gap-6 xl:grid-cols-1'>
          {addPoint ? (
            <MapPage
              addressPoint={pointAdd}
              plotcode={Plot}
              plotpoint={allPoint}
            />
          ) : (
            <MapPage plotcode={Plot} plotpoint={allPoint} />
          )}
        </div>
      </Main>
    </>
