          ...
          for (i = 0; i < data.length; i++) {
            ...
            if (isTable === 'securitie') {
              ...
              dataAsArray.push([
                ...
                ext.subZone,
                ext.manager,
                ext.promotion0_area_contract,
                ext.promotion0_ton_contract,
                ext.promotion0_budget_use,
                ext.promotion0_factor_status,

                ext.promotion1_area_contract,
                ext.promotion1_ton_contract,
                ext.promotion1_budget_use,
                ext.promotion1_factor_status,

                ext.promotion2_area_contract,
                ext.promotion2_ton_contract,
                ext.promotion2_budget_use,
                ext.promotion2_factor_status,

                ext.promotion3_area_contract,
                ext.promotion3_ton_contract,
                ext.promotion3_budget_use,
                ext.promotion3_factor_status,

                ext.promotion4_area_contract,
                ext.promotion4_ton_contract,
                ext.promotion4_budget_use,
                ext.promotion4_factor_status,

                ext.promotion5_area_contract,
                ext.promotion5_ton_contract,
                ext.promotion5_budget_use,
                ext.promotion5_factor_status,

                ext.promotion6_budget_use,
                ext.promotion7_budget_use,
                ext.updatedAt
              ]);
            }
          }
    ...
    if (isHour === true) {
      $('#list').navButtonAdd(`#${$grid[0].id}_toppager_left`, {
        ...
        async onClickButton() {
          ...
          const hours0 = 0;
          let day1;
          const day = new Date().toLocaleDateString('th-TH', {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
          });
          ...
          for (i = 0; i < 24; i++) {
            ...
            if (i < hours) {
              const num = 1;
              if (hours + i < 24) {
                day1 = `${hours + i}:00 - ${hours + i}:59`;
                // eslint-disable-next-line no-await-in-loop
                await axios
                  .post(
                    `${router.basePath}/api/${isTable}/new`,
                    { hourNum: num + i, days: day, hours: day1 },
                    config
                  )
                  .then((res) => {
                    if (res && res.status === 201) {
