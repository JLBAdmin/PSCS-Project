            ...
            _text : function(ctx, layer) {
                ...
                if (textStyle.stroke) {
                    ...
                    textArr.forEach((element,index) => {
                        // 统计字符串个数
                        let textOffset = element.split("").length * 8;
                        if(element.indexOf("—") != -1){
                            textOffset = element.split("").length * 13;
                        }
                        ctx.strokeText(element, p.x + offsetX - textOffset, p.y + offsetY + 25 * index);
                    });
                }
                ...
                if (textStyle.rotate) {//有旋转角度
                    ...
                    textArr.forEach((element,index) => {
                        ...
                        ctx.fillText(element, p.x + offsetX - textOffset, p.y + offsetY + 25 * index);
                    });
                }
            },
